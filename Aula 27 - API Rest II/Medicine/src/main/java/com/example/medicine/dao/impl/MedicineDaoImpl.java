...
public class MedicineDaoImpl implements IDao<Medicine> {
    ...
    public Medicine update(Medicine medicine) {
        Connection connection = settingJDBC.connectWithDatabase();
        Statement statement = null;
        String query = String.format("UPDATE medicines SET name='%s', laboratory='%s', " +
                        "quantity='%s', price='%s'", medicine.getName(), medicine.getLaboratory(),
    ...
    public void delete(Integer id) {
        Connection connection = settingJDBC.connectWithDatabase();
        Statement statement = null;
        String query = String.format("DELETE FROM medicines WHERE id='%s'", id);
